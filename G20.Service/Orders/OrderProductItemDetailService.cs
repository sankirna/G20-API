...
namespace G20.Service.Orders
{
    public class OrderProductItemDetailService : IOrderProductItemDetailService
    {
        ...
        public virtual async Task<OrderProductItemDetail> GetDetailsByOrderProductItemIdAsync(int orderProductItemId)
        {
            return await _entityRepository.Table.FirstOrDefaultAsync(x => x.OrderProductItemId == orderProductItemId);
        }
        ...
        public virtual async Task<OrderProductItemDetail> GetDetailsByQRCodeAsync(int productId,string qrCode)
        {   
            var ProductItemDetail = await _entityRepository.Table.FirstOrDefaultAsync(x => x.ProductId == productId && x.QRCode == qrCode);            
